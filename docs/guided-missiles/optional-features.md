...
近接信管は任意機能です。SACLOS／MCLOS Sampleでは再利用用の8機すべてに`FSE_ProximityFuze`が追加されていますが、`FSE_MissileController.FuzeModule`は未設定のため、既定では従来どおり接触時だけ起爆します。

使用する場合は、再利用するミサイルごとに次を設定します。

1. Sample以外の構成では、各ミサイルへ`FSE_ProximityFuze`を一つ追加します。SACLOS／MCLOS Sampleには追加済みです。
2. `Missile`へ同じミサイルの`FSE_MissileController`を指定します。
3. 同じミサイルの`FSE_MissileController.FuzeModule`へ、その`FSE_ProximityFuze`を指定します。
4. `DetonationRadius`、`ArmingDelaySeconds`、`ArmingDistance`を用途に合わせて設定します。

`ArmingDelaySeconds`と`ArmingDistance`の条件を両方満たした後に作動します。どちらかを0にすると、その条件だけを無効にできます。発射直後の誤作動を避けるため、母機や友軍から十分に離れる時間または距離を設定してください。
