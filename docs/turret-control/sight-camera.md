...
1. DialFunction用GameObjectへ`FSE_DFUNC_SightZoom`を追加します。
2. `SightController`へ`FSE_SightController`を指定し、操作席のDialFunctionへ登録します。
3. `MinimumFov`と`MaximumFov`で使用するFOV範囲を決めます。
4. 段階式を使う場合は`ZoomPresetMagnifications`を設定します。連続式を使う場合は`UsePresetSteps`を無効にします。
